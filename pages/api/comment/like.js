...
export default async function LikeHandler(요청,응답) {
    ...
    let comment = await db.collection('comment').findOne({_id : new ObjectId(요청.query.id)})
    //좋아요 누른 멤버 email
    session.user.email
    //좋아요 누른 댓글 id
    요청.query.id
    ...
    for (const likedCommentId of user.commentLikeList){
        ...
        if(요청.query.id == likedCommentId){
            ...
            return 응답.status(200).json({statusCode : 0, likedMembers : temp})
        }
    }
    ...
    응답.status(200).json({statusCode : 1, likedMembers : temp})
}